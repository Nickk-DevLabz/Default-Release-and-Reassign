...
function Refresh-RegistryData {
    ...
    try {
        ...
        # Build "Unassigned" Category (Items not in any hardcoded list)
        $AllMapped = $Categories.Values | ForEach-Object { $_ }
        $Global:Categories["Unassigned"] = $Extensions | Where-Object { $_ -notin $AllMapped }
        ...
        Write-Log "Failed to refresh: $_" "ERROR"
...
function Update-MasterList {
    ...
    $SelectedCat = if ($CategoryList.SelectedItem) { $CategoryList.SelectedItem } else { "All" }
    
    $DataGrid.Rows.Clear()

    foreach ($ext in $Extensions) {
        $MatchSearch = [string]::IsNullOrEmpty($SearchText) -or ($ext -like "*$SearchText*")
        ...
        if ($MatchSearch -and $MatchCat) {
            ...
            if ($assigned -ne "Windows Default") { 
...
$Categories = [ordered]@{
    "All"         = @()
    "Web/URL"     = @(".html",".htm",".url",".php",".webp",".css",".js","http","https","mailto")
    "Photos"      = @(".jpg",".jpeg",".png",".gif",".bmp",".tiff",".ico",".svg",".raw",".heic",".webp")
    "Video"       = @(".mp4",".mkv",".avi",".mov",".wmv",".flv",".3gp",".ts",".webm",".mpg",".mpeg")
    "Audio"       = @(".mp3",".wav",".flac",".m4a",".aac",".ogg",".wma",".mid",".midi",".amr")
    "Documents"   = @(".pdf",".txt",".rtf",".docx",".doc",".odt",".xlsx",".xls",".csv",".pptx",".ppt")
    "Compress"    = @(".zip",".rar",".7z",".tar",".gz",".iso",".cab",".bz2")
    "Contacts"    = @(".vcf",".contact",".vcard")
    "MS Extras"   = @(".msi",".diagcab",".appref-ms",".application",".ps1",".bat",".reg")
    "Unassigned"  = @()
}

$Form = New-Object System.Windows.Forms.Form
$Form.Text = "WinClean Pro v7.8 - Association Architect"; $Form.Size = "850, 750"; $Form.StartPosition = "CenterScreen"
...
$BtnOpenTemp = New-Object System.Windows.Forms.Button -Property @{Text="Open Temp Folder"; Location="20,160"; Size="200,40"}
$BtnCleanTemp = New-Object System.Windows.Forms.Button -Property @{Text="Purge Picker Temp Files"; Location="20,210"; Size="200,40"; BackColor="Salmon"}

$ToolsTab.Controls.AddRange(@($LblBackupPath, $BtnSetPath, $BtnRestorePoint, $BtnOpenTemp, $BtnCleanTemp))
...
$BtnSetPath.Add_Click({
    ...
    if ($FolderBrowser.ShowDialog() -eq "OK") {
        ...
        Write-Log "Backup path changed."
    }
})

$BtnOpenTemp.Add_Click({
...
$BtnCleanTemp.Add_Click({
    ...
    Write-Log "Cleaned $count temporary picker files from $env:TEMP"
    [System.Windows.Forms.MessageBox]::Show("Removed $count temporary files.", "Cleanup")
})
    ...
    foreach($row in $selectedRows) {
        ...
        try {
            ...
            if ($proc.ExitCode -eq 0) {
                ...
                if (Test-Path $regPath) { Remove-Item $regPath -Force; Write-Log "Released $ext" }
            }
        } catch { Write-Log "Error processing ${ext}: $_" "ERROR" }
...
$BtnAssign.Add_Click({
    ...
    foreach($row in $selectedRows) {
        $ext = $row.Cells["Ext"].Value
        Set-Status "Waiting for Picker: $ext"
...
Refresh-RegistryData
foreach ($cat in $Categories.Keys) { [void]$CategoryList.Items.Add($cat) }
