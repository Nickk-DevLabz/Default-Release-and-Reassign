...
$BtnRelease.Add_Click({
    ...
    foreach($row in $DataGrid.Rows) {
        if ($row.Cells["Check"].Value -eq $true) {
            ...
            $file = "$DefaultBackupPath\Backup_$($ext.Replace('.',''))_$(Get-Date -Format 'HHmmss').reg"
            Start-Process "reg.exe" -ArgumentList "export `"HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\FileExts\$ext`" `"$file`"" -Wait -WindowStyle Hidden
...
$BtnAssign.Add_Click({
    foreach($row in $DataGrid.Rows) {
        if ($row.Cells["Check"].Value -eq $true) {
            ...
            $tempFile = "$env:TEMP\winclean_temp$ext"
            if (!(Test-Path $tempFile)) { New-Item $tempFile -ItemType File -Force }
